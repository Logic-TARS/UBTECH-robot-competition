...
CONFIG = {
    "width": 1280,
    "height": 720,
    "headless": os.environ.get("UBTECH_HEADLESS", "false").lower() == "true",
}
...
# ── 1. Configuration ─────────────────────────────────────────────────
default_config = os.path.join(os.path.dirname(__file__), "config/Part_Sorting.yaml")
config_path = os.environ.get("UBTECH_TASK_CONFIG", default_config)
cfg = load_config(config_path)
grasp_cfg = cfg.get("grasp", {})
task2_fsm_visual = cfg.get("task_number") == 2 and cfg.get("fsm_mode", False)
...
if task2_fsm_visual:
    from source.conveyor_spawner import ConveyorPartSpawner

    spawner = ConveyorPartSpawner(cfg, scene, world)
    spawner.reset()

    def conveyor_visual_callback(step_size):
        spawner.update()

    world.add_physics_callback("conveyor_visual", conveyor_visual_callback)
    print("[Init] Task2 FSM 可视化模式：动态生成零件并沿传送带移动")
else:
    ...
    robot.initialize()
    print("[Init] 机器人 articulation 已初始化并设置初始关节角")
    ...
    coord_transform = CoordinateTransform.from_torso_link(
        ik_solver=robot.ik_solver,
