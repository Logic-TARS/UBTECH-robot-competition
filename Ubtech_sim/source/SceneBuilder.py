...
class SceneBuilder:
    ...
    def init_coordinate_transform(self, ik_solver, torso_prim_path: str | None = None) -> None:
    ...
    def check_success(self, tolerance_xy: float = 0.85) -> bool:
        """Task 1 success check: verify all Part A/B objects are in their correct box regions.

        The box is split along its Y-axis center:
          - Part A must land in Y < box_y  (left region)
          - Part B must land in Y >= box_y (right region)

        A part is considered "in box" when its X and Y coordinates fall within the
        box extents (scaled by tolerance_xy).

        Args:
            tolerance_xy: factor applied to box half-extents for XY boundary check.
                          Default 0.85 means a part must be within 85% of the half-extent
                          from the box center to count as "inside".

        Returns:
            True if all parts are placed in their correct regions.
        """
        import logging
        logger = logging.getLogger(__name__)

        if self.cfg.get("task_number") != 1:
            logger.warning("check_success: only implemented for Task 1 (Part_Sorting)")
            return False

        box_cfg = self.cfg.get("box", {})
        box_positions = box_cfg.get("box_position", [])
        if not box_positions:
            logger.warning("check_success: no box_position in config")
            return False
        box_pos = np.array(box_positions[0], dtype=float)
        box_scale = np.array(box_cfg.get("box_scale", [[0.38, 0.75, 0.36]])[0], dtype=float)
        half_x = box_scale[0] / 2.0 * tolerance_xy
        half_y = box_scale[1] / 2.0 * tolerance_xy

        poses = self.get_parts_world_poses()
        if not poses:
            logger.warning("check_success: no parts found")
            return False

        for pose in poses:
            prim_path = pose.get("prim_path", "")
            part_type = self.part_type_by_prim_path.get(prim_path)
            if part_type is None:
                logger.warning(f"check_success: unknown part type for {prim_path}")
                return False

            pos = np.array(pose["position"], dtype=float)
            in_x = abs(pos[0] - box_pos[0]) <= half_x
            in_y = abs(pos[1] - box_pos[1]) <= half_y
            if not (in_x and in_y):
                logger.info(
                    f"check_success: {prim_path} ({part_type}) outside box bounds "
                    f"pos=({pos[0]:.4f}, {pos[1]:.4f}) box=({box_pos[0]:.4f}, {box_pos[1]:.4f})"
                )
                return False

            y_correct = (part_type == "part_a" and pos[1] < box_pos[1]) or (
                part_type == "part_b" and pos[1] >= box_pos[1]
            )
            if not y_correct:
                logger.info(
                    f"check_success: {prim_path} ({part_type}) in wrong region "
                    f"pos_y={pos[1]:.4f} box_y={box_pos[1]:.4f}"
                )
                return False

        logger.info(f"check_success: all {len(poses)} parts correctly placed")
        return True
