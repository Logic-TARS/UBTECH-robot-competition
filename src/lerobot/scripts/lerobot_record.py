...
def record_loop(
    ...
    ACTION_CHUNK_EXEC = 50
    no_action_count = 0
    timestamp = 0
    start_episode_t = time.perf_counter()
    loop_count = 0
    action_step_idx = 0
    robot._robot_interface._smooth_alpha = 0.3
    while timestamp < control_time_s:
        ...
        if policy is not None and preprocessor is not None and postprocessor is not None:
            if action_step_idx == 0:
                ...
                action_plan = action_chunk[0, :ACTION_CHUNK_EXEC, :]

            action_values = action_plan[action_step_idx : action_step_idx + 1]
            action_step_idx += 1
            if action_step_idx >= ACTION_CHUNK_EXEC:
                policy.reset()
                action_step_idx = 0
