...
from src.lerobot.policies.rtc.modeling_rtc import RTCProcessor
from src.lerobot.policies.smolvla.configuration_smolvla import SmolVLAConfig
from src.lerobot.policies.smolvla.smolvlm_with_expert import SmolVLMWithExpertModel
from src.lerobot.policies.utils import (
    populate_queues,
)
from src.lerobot.utils.constants import ACTION, OBS_ENV_STATE, OBS_LANGUAGE_ATTENTION_MASK, OBS_LANGUAGE_TOKENS, OBS_STATE
from src.lerobot.utils.utils import get_safe_dtype
...
class SmolVLAPolicy(PreTrainedPolicy):
    ...
    def prepare_state(self, batch):
        """Pad state, optionally with environment_state concatenation."""
        state = batch[OBS_STATE][:, -1, :] if batch[OBS_STATE].ndim > 2 else batch[OBS_STATE]
        if self.config.use_environment_state:
            env_state = batch.get(OBS_ENV_STATE)
            if env_state is None:
                raise KeyError(
                    f"use_environment_state=true but {OBS_ENV_STATE} not in batch. "
                    f"Keys: {list(batch.keys())}"
                )
            if env_state.ndim > 2:
                env_state = env_state[:, -1, :]
            state = torch.cat([state, env_state], dim=-1)
