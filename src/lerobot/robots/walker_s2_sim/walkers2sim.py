...
class WalkerS2sim(Robot):
    ...
    def connect(self, calibrate: bool = True) -> None:
        ...
        try:
            ...
            logger.info("Replicator operations flushed")

            self._world.play()
            settle_steps = int(2.0 / self.config.physics_dt)
            for _ in range(settle_steps):
                self._world.step(render=False)
            logger.info(f"Physics settled ({settle_steps} steps)")

            self._world.pause()
            logger.info("World paused for robot loading")

            self._scene_builder.build_robot()
            logger.info("SceneBuilder robot loaded")

            self._world.play()
            for _ in range(50):
                self._world.step(render=False)
            logger.info("Physics warmup complete (50 steps)")
        ...
        logger.info(f"使用 prim_path: {actual_prim_path}")
        ...
        if self._scene_builder is not None:
            torso_path = f"{actual_prim_path}/torso_link"
            self._scene_builder.init_coordinate_transform(
                self._robot_interface.ik_solver,
                torso_prim_path=torso_path,
            )
            logger.info(f"Coordinate transform initialized (torso: {torso_path})")
