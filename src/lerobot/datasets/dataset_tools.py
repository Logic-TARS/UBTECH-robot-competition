...
from src.lerobot.datasets.aggregate import aggregate_datasets
...
from src.lerobot.datasets.video_utils import encode_video_frames, get_video_info
from src.lerobot.utils.constants import HF_LEROBOT_HOME, OBS_IMAGE
...
def _copy_and_reindex_data(
    ...
    for old_idx in episode_mapping:
        try:
            ...
            logging.error(f"Error getting data file path for episode old_idx={old_idx}")
            raise
    ...
    for src_path in tqdm(sorted(file_to_episodes.keys()), desc="Processing data files"):
        try:
            ...
            logging.error(f"Error reading parquet file: {src_path}")
            raise
        ...
        for ep_old_idx in episodes_to_keep:
            ...
            try:
                ...
                logging.error(
                    f"Error building data metadata for episode old_idx={ep_old_idx}, "
                    f"new_idx={ep_new_idx}, file={src_path}"
                )
                raise
    ...
    for video_key in src_dataset.meta.video_keys:
        ...
        for old_idx in episode_mapping:
            try:
                ...
                logging.error(
                    f"Error building video file mapping for episode old_idx={old_idx}, "
            ...
            if all_in_file_set == episodes_to_keep_set:
                ...
                for old_idx in episodes_in_file:
                    ...
                    try:
                        ...
                        logging.error(
                            f"Error copying video metadata for episode old_idx={old_idx}, "
                            f"new_idx={new_idx}, video_key={video_key}"
                        )
                        raise
            else:
                ...
                for old_idx in sorted_keep_episodes:
                    try:
                        ...
                        assert src_ep["length"] == to_frame - from_frame, (
                            f"Episode length mismatch for episode old_idx={old_idx}: "
                            f"{src_ep['length']} vs {to_frame - from_frame}"
                        )
                        episodes_to_keep_ranges.append((from_frame, to_frame))
                    except Exception:
                        logging.error(
                            f"Error building frame range for episode old_idx={old_idx}, "
                            f"video_key={video_key}"
                        )
                        raise
                ...
                for old_idx in sorted_keep_episodes:
                    ...
                    try:
                        ...
                        logging.error(
                            f"Error building re-encoded video metadata for episode old_idx={old_idx}, "
                            f"new_idx={new_idx}, video_key={video_key}"
                        )
                        raise
...
def _copy_and_reindex_episodes_metadata(
    ...
    """
    from src.lerobot.datasets.utils import flatten_dict
    ...
    for old_idx, new_idx in tqdm(
        ...
        print(f"Processing episode old_idx={old_idx}, new_idx={new_idx}")
        try:
            src_episode_full = _load_episode_with_stats(src_dataset, old_idx)
        except Exception:
            logging.error(f"Error loading stats for episode old_idx={old_idx}, new_idx={new_idx}")
            raise
        ...
        try:
            ...
            logging.error(f"Error saving metadata for episode old_idx={old_idx}, new_idx={new_idx}")
            raise
...
def _write_parquet(df: pd.DataFrame, path: Path, meta: LeRobotDatasetMetadata) -> None:
    ...
    """
    from src.lerobot.datasets.utils import embed_images, get_hf_features_from_features
...
def _copy_data_without_images(
    ...
    """
    from src.lerobot.datasets.utils import DATA_DIR
