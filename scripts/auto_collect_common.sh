...
AUTO_COLLECT_ENTRY="${AUTO_COLLECT_ENTRY:-src/lerobot/scripts/auto_collect_main.py}"
...
/isaac-sim/python.sh ${AUTO_COLLECT_ENTRY} \
    ...
    --auto_collect.task=${TASK} \
    --auto_collect.root=${ROOT_DIR} \
    --auto_collect.repo_id=${REPO_ID} \
    --auto_collect.num_episodes=${NUM_EPISODES} \
    --auto_collect.fps=${FPS} \
    --auto_collect.video=${VIDEO} \
    --auto_collect.objects_per_episode=${OBJECTS_PER_EPISODE} \
    --auto_collect.single_task='${SINGLE_TASK}' \
    --auto_collect.record_data=${RECORD_DATA} \
    --auto_collect.push_to_hub=${PUSH_TO_HUB}"
