"""PickPlace FSM Agent — thin wrapper that wires SceneBuilder and RobotInterface into PickPlaceFSM.

Called from ``_robot_control_callback`` on every physics step.  Returns
end-effector targets and gripper commands that the callback feeds into the
existing IK pipeline.
"""
...
from lerobot.common.robot_devices.fsm.pick_place_fsm import PickPlaceFSM

logger = logging.getLogger(__name__)


class PickPlaceFSMAgent:
    """Agent that drives a :class:`PickPlaceFSM` each physics step.

    Responsibilities:
    1. Query current EE poses from the robot interface.
    2. Query current part world positions from the SceneBuilder.
    3. Call ``fsm.step()`` and return the targets / gripper commands.
    4. Manage FSM lifecycle (start, reset, pause/resume).

    Args:
        fsm: Initialized :class:`PickPlaceFSM` instance.
    ...
    def __init__(self, fsm: PickPlaceFSM, scene_builder, robot_interface):
        self._fsm = fsm
    ...
    def start(self) -> None:
        """Start the FSM (query part count from SceneBuilder)."""
        if self._started:
            return
        # Determine number of parts
        part_poses = self._query_part_poses()
        num_parts = len(part_poses) if part_poses else 4
        ...
        self._started = True
        self._paused = False
        logger.info(f"[PickPlaceFSMAgent] Started with {num_parts} parts")

    def pause(self) -> None:
        """Pause the FSM (it still runs queries but outputs nothing)."""
        self._paused = True
    ...
    def reset(self) -> None:
        """Reset the FSM for a new episode."""
        self._fsm.reset()
        self._started = False
        self._paused = True
    ...
    @property
    def fsm(self) -> PickPlaceFSM:
        """Direct access to the underlying FSM (for querying state / IK params)."""
        return self._fsm
    ...
    def _query_ee_poses(self) -> Optional[dict]:
        ...
        try:
            ...
            logger.warning(f"[PickPlaceFSMAgent] Failed to get EE poses: {e}")
            return None
    ...
    def _query_part_poses(self) -> Optional[list]:
        ...
        try:
            ...
            logger.warning(f"[PickPlaceFSMAgent] Failed to get part poses: {e}")
            return None
    ...
    def _query_gripper_width(self) -> Optional[float]:
        ...
        try:
            ...
            logger.debug(f"[PickPlaceFSMAgent] Failed to get gripper width: {e}")
            return None