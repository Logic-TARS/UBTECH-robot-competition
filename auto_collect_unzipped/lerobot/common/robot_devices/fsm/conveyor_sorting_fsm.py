...
class ConveyorSortingFSM:
    ...
    def __init__(
        ...
        self._grasp_depth_offset: float = getattr(config, "fsm_grasp_depth_offset", -0.02)  # How far below part center to grasp
    ...
    def ik_rot_weight(self):
        ...
        if self._state in [ConveyorFSMState.TRANSPORT, ConveyorFSMState.LOWER, ConveyorFSMState.RETREAT]:
    ...
    def _compute_and_cache_target(self) -> None:
        ...
        if self._state == ConveyorFSMState.WAIT_FOR_PART:
            ...
            if target is not None:
                ...
                has_converged = self._is_converged_custom(target, self._pos_tol * 1.0)  # 3cm tolerance
                min_descent_frames = 50  # Minimum ~1.7s at 30Hz to allow full descent
                
                if has_converged and self._frame_counter >= min_descent_frames:
                    ...
                    if self._frame_counter == 0:
                        logger.info(
                            ...
                            f"Need {min_descent_frames} frames minimum, convergence tolerance={self._pos_tol*100:.1f}cm"
                        )
    ...
    def _grasp_pos(self) -> Optional[np.ndarray]:
        ...
        if self._grasped_part_position is not None:
            ...
            is_retry = self._failed_parts.get(self._current_part_index, 0) > 0
            if is_retry:
                part_world = self._predict_part_position(lookahead_time=0.15)
                if part_world is None:
                    part_world = self._get_current_part_world()
            else:
                part_world = self._get_current_part_world()
        ...
        # Descend below part center to ensure gripper surrounds the part properly
        # This accounts for the gripper finger geometry and ensures full contact
        # Default: configurable via fsm_grasp_depth_offset (typically -0.02 to -0.07)
    ...
    def _check_transition(self) -> None:
        ...
        if self._state == ConveyorFSMState.WAIT_FOR_PART and self._state_frame_counter % 200 == 0:
            logger.warning(
            ...
            logger.warning(f"[ConveyorFSM-DEBUG] Part positions: {self._part_positions_world}")
            logger.warning(f"[ConveyorFSM-DEBUG] Grab zone: [{self._grab_zone_x_min}, {self._grab_zone_x_max}]")
            logger.warning(f"[ConveyorFSM-DEBUG] Has available part: {self._has_available_part()}")
        ...
        if self._state == ConveyorFSMState.WAIT_FOR_PART:
            ...
            if target is not None:
                ...
                # AND require minimum descent time to ensure actual movement
                has_converged = self._is_converged_custom(target, self._pos_tol * 1.0)  # 3cm tolerance
                min_descent_frames = 5  # ~0.17s at 30Hz — minimize conveyor dwell

                # Safety fallback: if IK stays stuck (right_success=False persists),
                # _is_converged will never be True. Close gripper anyway after this
                # many frames so the FSM does not deadlock. ~1.3s at 30Hz.
                grasp_ik_stuck_frames = 40
                force_close = self._frame_counter >= grasp_ik_stuck_frames
                if force_close and self._frame_counter == grasp_ik_stuck_frames:
                    logger.warning(
                        f"[ConveyorFSM-GRASP] IK did not converge after "
                        f"{grasp_ik_stuck_frames} frames, force-closing gripper."
                    )
                ...
                if (has_converged and self._frame_counter >= min_descent_frames) or force_close:
                    ...
                    gripper_close_frames = 8  # ~0.27s for gripper to fully clamp + settle — prevents Part A flinging
                    if self._frame_counter >= min_descent_frames + gripper_close_frames:
                        logger.info(
                            f"[ConveyorFSM] GRASP complete ({self._frame_counter} frames), "
                            f"transitioning to LIFT (verify-after-lift)"
                        )
            ...
            if self._is_converged(target) or timed_out:
                ...
                logger.info(
                    f"[ConveyorFSM] LIFT complete, transitioning to VERIFY_GRASP"
                )
    ...
    def _has_available_part(self) -> bool:
        ...
        Part A gets a 5cm earlier x_min because it's larger and needs more
        grasp time — this prevents selecting Part A too late.
        ...
        for idx, part_pos in enumerate(self._part_positions_world):
            ...
            if idx in self._skipped_parts:
                continue
            part_type = self._identify_part_type(idx)
            x_min = self._grab_zone_x_min - 0.05 if part_type == 'part_a' else self._grab_zone_x_min
            if x_min <= part_pos[0] <= self._grab_zone_x_max:
                return True
        return False
    ...
    def _select_next_part(self) -> None:
        ...
        Strategy:
        - Find parts within grab zone (Part A: 5cm earlier x_min)
        ...
        for idx, part_pos in enumerate(self._part_positions_world):
            ...
            # Part-A-specific earlier selection (larger part needs more time)
            part_type = self._identify_part_type(idx)
            x_min = self._grab_zone_x_min - 0.05 if part_type == 'part_a' else self._grab_zone_x_min
            
            # Check if in grab zone
            if part_pos[0] < x_min or part_pos[0] > self._grab_zone_x_max:
                continue
    ...
    def _is_part_in_grasp_position(self) -> bool:
        ...
        Part A is larger and needs more time to descend/clamp, so it starts
        approaching 5cm earlier on the conveyor.
        """
        part_world = self._get_current_part_world()
        if part_world is None:
            return False

        # Part-A-specific earlier trigger: larger part needs more grasp time.
        x_min = self._grab_zone_x_min
        if self._current_part_type == 'part_a':
            x_min = self._grab_zone_x_min - 0.05  # Start 5cm earlier (~0.5s extra)

        return x_min <= part_world[0] <= self._grab_zone_x_max
