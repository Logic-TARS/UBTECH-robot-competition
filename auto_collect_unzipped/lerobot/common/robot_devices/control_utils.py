...
def control_loop(
    ...
    while timestamp < control_time_s:
        ...
        try:
            ...
            # Check if FSM task is done (autonomous data collection)
            # For Conveyor_Sorting, exit early when task is done so we can evaluate episode success
            # NOTE: is_task_done is a @property, not a method, so hasattr (not has_method) must be used.
            if hasattr(robot, "is_task_done") and robot.is_task_done:
                logger.info(f"is_task_done=True at timestamp={timestamp:.2f}s, exiting loop")
                events["exit_early"] = True
                break
            elif loop_count % 100 == 1 and hasattr(robot, "is_task_done"):
                logger.debug(f"is_task_done check: value={robot.is_task_done}, FSM state={getattr(getattr(robot, '_fsm_agent', None), 'current_state', 'N/A')}")
