...
class IsaacSimRobotInterface:
    ...
    def reset(self):
        ...
        """
        # Check if physics handle is valid before resetting
        # After scene reset, the articulation might need re-initialization
        # Try up to 5 times with small delays to wait for physics to initialize
        max_retries = 5
                ...
                if hasattr(self._articulation, 'is_physics_handle_valid'):
                    if not self._articulation.is_physics_handle_valid():
                        if attempt < max_retries - 1:
                            logger.warning(
                                f"[reset] Physics handle invalid, attempt {attempt+1}/{max_retries}, retrying..."
                            )
                            import time
                            ...
                            logger.error(
                                f"[reset] CRITICAL: Physics handle invalid after {max_retries} attempts!"
                            )
                            return
                elif hasattr(self._articulation, '_physics_view'):
                    if self._articulation._physics_view is None:
                        if attempt < max_retries - 1:
                            logger.warning(
                                f"[reset] _physics_view is None, attempt {attempt+1}/{max_retries}, retrying..."
                            )
                            import time
                            time.sleep(0.1)  # Wait 100ms
                            continue
                            ...
                            logger.error(
                                f"[reset] CRITICAL: _physics_view is None after {max_retries} attempts!"
                            )
                            return
                else:
                    # No validity check available, assume it's OK
                    break
                    
                # If we get here, the handle is valid
                break
            except Exception as e:
                if attempt < max_retries - 1:
                    logger.warning(f"[reset] Error checking physics handle (attempt {attempt+1}): {e}")
                    import time
                    time.sleep(0.1)
                    continue
                else:
                    logger.error(f"[reset] CRITICAL: Failed to check physics handle: {e}")
                    return
    ...
    def get_joint_states(self):
        ...
        # Check if physics handle is valid before accessing articulation
        # After scene reset or prim deletion, _physics_view might be None
        # In this case, we need to reinitialize the articulation
        physics_valid = False
        try:
            if hasattr(self._articulation, 'is_physics_handle_valid'):
                physics_valid = self._articulation.is_physics_handle_valid()
            elif hasattr(self._articulation, '_physics_view'):
                physics_valid = self._articulation._physics_view is not None
            else:
                physics_valid = True  # Assume valid if we can't check
        except Exception:
            physics_valid = False

        if not physics_valid:
            logger.warning("[get_joint_states] Physics handle invalid, reinitializing articulation...")
            try:
                self.reinitialize_articulation()
                ...
                if not physics_valid:
                    logger.error("[get_joint_states] Still invalid after reinitialization, returning None")
                    return None
                else:
                    logger.info("[get_joint_states] Articulation reinitialized successfully")
            except Exception as e:
                logger.error(f"[get_joint_states] Failed to reinitialize articulation: {e}")
                return None
    ...
    def _smooth_joints(self, side: str, ik_positions: np.ndarray, alpha: float | None = None) -> np.ndarray:
        ...
        # Guard: if IK returned NaN, hold previous position to avoid poisoning the cache
