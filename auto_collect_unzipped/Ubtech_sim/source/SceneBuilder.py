...
class SceneBuilder:
    ...
    def build_parts(self):
        ...
        if self.cfg['task_number'] == 3:
            ...
            # 初始化 Cloner 供后续动态生成使用
            self.cloner = Cloner()
            
            # Check if using dynamic spawning mode (FSM mode)
            use_dynamic_spawning = self.cfg.get('grasp', {}).get('total_parts_to_sort') is not None
            
            if use_dynamic_spawning:
                # Dynamic spawning mode: don't create parts yet, spawner will do it
                self.parts_prim_paths = []
                print("[SceneBuilder] Task2 初始化: 动态生成模式 (FSM)")
            else:
                # Legacy mode: create all parts at once (original behavior)
                ...
                print(f"[SceneBuilder] Task2 初始化: 发现 {len(self.parts_prim_paths)} 个零件 (静态模式)")
    ...
    def get_parts_world_poses(self):
        ...
        Returns:
            list of dict: [{'prim_path': str,
                            ...
                            'part_type': str}, ...]
            part_type is 'part_a' or 'part_b', identified via semantics or prim path.
        """
        from pxr import UsdGeom, Usd, Semantics
        import omni.usd
        stage = omni.usd.get_context().get_stage()
        ...
        results = []
        for idx, prim_path in enumerate(self.parts_prim_paths):
            try:
                ...
                # Identify part type from semantics or prim path substring
                part_type = self._identify_part_type(prim, prim_path)
                ...
                # Log part type once per episode (first call after each reset/new episode)
                if idx == 0 and not getattr(self, '_part_type_episode_logged', False):
                    self._part_type_episode_logged = True
                    self._pending_part_type_log = True
                if getattr(self, '_pending_part_type_log', False):
                    child_names = [str(c.GetName()) for c in prim.GetChildren()]
                    print(f"[SceneBuilder] Part [{idx}] type={part_type}, children={child_names[:5]}")
                    if idx == len(self.parts_prim_paths) - 1:
                        self._pending_part_type_log = False
    ...
    def get_part_types(self) -> list:
        """Get list of part types for all parts in the scene.
        
        Returns:
            List of part types: ['part_a', 'part_b', ...]
            Returns empty list if parts are not yet initialized.
        """
        if not hasattr(self, 'parts_prim_paths') or not self.parts_prim_paths:
            return []
        
        # Try to get types from current poses if already computed
        try:
            poses = self.get_parts_world_poses()
            if poses:
                return [p.get('part_type', None) for p in poses]
        except Exception as e:
            print(f"[SceneBuilder] Failed to get part types from poses: {e}")
        
        # Fallback: infer from prim paths
        part_types = []
        for prim_path in self.parts_prim_paths:
            part_type = self._identify_part_type_from_path(prim_path)
            part_types.append(part_type)
        
        return part_types
    
    @staticmethod
    def _identify_part_type_from_path(prim_path: str) -> str:
        """Identify part type from prim path string alone (no prim object needed).
        
        Args:
            prim_path: USD prim path string
            
        Returns:
            'part_a', 'part_b', or None if cannot determine
        """
        path_lower = str(prim_path).lower()
        
        # Check for part_a indicators
        if 'part_a' in path_lower or 'parta' in path_lower or \
           'tn__0502_0326_asm_asm_bp' in path_lower or \
           'task1_parta' in path_lower:
            return 'part_a'
        
        # Check for part_b indicators
        if 'part_b' in path_lower or 'partb' in path_lower or \
           'left_leg_a_asm_1_asm' in path_lower or \
           'part_b.usd' in path_lower:
            return 'part_b'
        
        return None

    @staticmethod
    def _identify_part_type(prim, prim_path: str) -> str:
        """Identify whether a part is 'part_a' or 'part_b'.

        Strategy:
        1. Check USD semantics (set during rep.create.from_usd)
        2. Walk children/descendants for known prim path substrings
           - 'tn__0502_0326_ASM_ASM_bP' → part_a (Task1_PartA asset)
           - 'LEFT_LEG_A_ASM_1_ASM' → part_b (Part_B asset)
        3. Fallback: index-based (first num_parts = part_a, rest = part_b)
        """
        # Strategy 1: check semantics on this prim and ancestors
        try:
            from pxr import Semantics
            check_prim = prim
            for _ in range(5):  # search up to 5 levels
                if check_prim is None or not check_prim.IsValid():
                    break
                api = check_prim.GetAPI(Semantics.SemanticsAPI)
                if api:
                    attr = api.GetSemanticLabelAttr()
                    if attr:
                        label = attr.Get()
                        if 'part_a' in str(label).lower():
                            return 'part_a'
                        elif 'part_b' in str(label).lower():
                            return 'part_b'
                check_prim = check_prim.GetParent()
        except Exception:
            pass

        # Strategy 2: match prim path itself
        path_lower = prim_path.lower()
        if 'left_leg_a_asm' in path_lower:
            return 'part_b'
        elif 'tn__0502' in path_lower:
            return 'part_a'

        # Strategy 3: walk descendants looking for known child prim names
        #   Replicator creates Ref_Xform_0N as parent; the actual part USD
        #   is loaded as a child with the original assembly name.
        def _walk_descendants(p, depth=0):
            """Recursively walk children to find known identifiers."""
            if depth > 6:
                return None
            for child in p.GetChildren():
                child_path = str(child.GetPath()).lower()
                if 'left_leg_a_asm' in child_path:
                    return 'part_b'
                elif 'tn__0502' in child_path:
                    return 'part_a'
                # Recurse deeper
                result = _walk_descendants(child, depth + 1)
                if result is not None:
                    return result
            return None

        desc_result = _walk_descendants(prim)
    ...
    def spawn_conveyor_part(self, part_type: str, position: list, rotation: list = None):
        """Spawn a single part on conveyor for Task 2 dynamic mode.
        
        Args:
            part_type: 'A' or 'B'
            position: [x, y, z] world position
            rotation: [rx, ry, rz] euler angles (optional, random if None)
            
        Returns:
            prim_path of spawned part
        """
        import omni.usd
        ...
        try:
            ...
            if prim.IsValid():
                # Apply RigidBodyAPI and enable
                rb_api = UsdPhysics.RigidBodyAPI.Apply(prim)
                rb_api.CreateRigidBodyEnabledAttr(True)
                # Apply MassAPI
                mass_api = UsdPhysics.MassAPI.Apply(prim)
                mass_api.CreateMassAttr(0.2)  # Set mass to 0.2kg
                
                # Set visibility explicitly for the prim and all children
                imageable = UsdGeom.Imageable(prim)
                imageable.CreateVisibilityAttr("inherited")
                
                # Also set visibility for all child prims (meshes, etc.)
                for child in prim.GetChildren():
                    if child.IsValid():
                        child_imageable = UsdGeom.Imageable(child)
                        child_imageable.CreateVisibilityAttr("inherited")
    ...
    def delete_spawned_conveyor_parts(self):
        """Delete all dynamically spawned conveyor parts."""
        import omni.usd
        stage = omni.usd.get_context().get_stage()
    ...
    def _scatter_parts_direct(self, plane_index=0, prim_paths=None):
        ...
        # Reset the per-episode logging flag so get_parts_world_poses prints types again
        self._part_type_episode_logged = False
    ...
    def reset(self):
        ...
        if task == 2:
            ...
            # Check if rigid_prim exists (only for old Task2, not Conveyor_Sorting with FSM)
            if hasattr(self, 'rigid_prim') and self.rigid_prim is not None:
                # Old Task2 approach: reset using RigidPrim
                ...
                # Conveyor_Sorting with FSM: parts are dynamically spawned by ConveyorPartSpawner
                # Just clean up any remaining spawned parts, spawner will handle new episode
